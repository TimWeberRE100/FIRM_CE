...
class Scenario:
    ...
    def _get_scenario_dicts(self, imported_dict: Dict[str, Dict[str, str]]) -> Dict[str, str]:
        ...
        return {
            idx: imported_dict[idx] for idx in imported_dict if self.name in parse_comma_separated(
                imported_dict[idx]['scenarios']
            )
        }
    ...
    def _get_datafiles(self, all_datafiles: Dict[str, Dict[str, str]]) -> Dict[str, DataFile]:
        ...
        return {
            idx: DataFile(
                all_datafiles[idx]['filename'],
                all_datafiles[idx]['datafile_type']
            ) for idx in all_datafiles if self.name in parse_comma_separated(all_datafiles[idx]['scenarios'])
        }
