...
class Statistics:
    def __init__(
        ...
        print(
            f"{scenario_name} LCOE: {self.solution.lcoe} [$/MWh], Penalties: {self.solution.penalties},"
        ...
        self.results_directory = self.create_solution_directory(
            ...
            scenario_name+'_'+balancing_type
        )
    ...
    def copy_temp_files(self, copy_callback: bool) -> None:
        if copy_callback:
            ...
            if SAVE_POPULATION:
                shutil.copy(
                    os.path.join(temp_dir, "population.csv"),
                    os.path.join(self.results_directory, "population.csv")
                )
                shutil.copy(
    ...
    def get_asset_column_count(self, include_minor_lines: bool = True, include_energy_limits: bool = True) -> int:
        return len(self.solution.fleet.generators) \
    ...
    def dump(self):
        ...
        residual_load_data = np.array(
            [node.residual_load for node in self.solution.network.nodes.values()],
            dtype=np.float64,
        ).T
        ResultFile(
            'residual_load',
            self.results_directory,
            residual_load_header,
            residual_load_data
        ).write()
        ResultFile(
            'block_lengths',
