    ...
    for node in network.nodes.values():
        ...
        for idx, storage_order in enumerate(node.storage_merit_order[::-1]):
            ...
            dispatch_power_update = min(
                max(-node.existing_surplus, -fleet.storages[storage_order].charge_max_t),
                0.0
            )
        ...
        for idx, storage_order in enumerate(node.storage_merit_order[::-1]):
            ...
            if fleet.storages[storage_order].trickling_flag:
                dispatch_power_update = max(
                    min(intranode_trickle, fleet.storages[storage_order].discharge_max_t),
                    0.0
                )
                storage_m.update_precharge_dispatch(
                    fleet.storages[storage_order],
            ...
            if fleet.storages[storage_order].precharge_flag:
                dispatch_power_update = min(
                    max(intranode_precharge, -fleet.storages[storage_order].charge_max_t),
                    0.0
                )
                storage_m.update_precharge_dispatch(
                    fleet.storages[storage_order],
...
def perform_internode_interstorage_transfers(
    ...
    for node in network.nodes.values():
        ...
        for idx, storage_order in enumerate(node.storage_merit_order[::-1]):
            ...
            if fleet.storages[storage_order].trickling_flag:
                dispatch_power_update = max(
                    min(node.imports_exports_update, fleet.storages[storage_order].discharge_max_t),
                    0.0
                )
                storage_m.update_precharge_dispatch(
                    fleet.storages[storage_order],
...
def perform_intranode_flexible_transfers(
    ...
    for node in network.nodes.values():
        ...
        for idx, flexible_order in enumerate(node.flexible_merit_order):
            ...
            dispatch_power_update = max(
                min(intranode_trickle, fleet.generators[flexible_order].flexible_max_t),
                0.0
            )
    ...
    for node in network.nodes.values():
        for idx, flexible_order in enumerate(node.flexible_merit_order):
            if not fleet.generators[flexible_order].trickling_flag:
                continue
            dispatch_power_update = max(
                min(node.imports_exports_update, fleet.generators[flexible_order].flexible_max_t),
                0.0
            )
            generator_m.update_precharge_dispatch(
                fleet.generators[flexible_order],
