...
def create_dynamic_copy(
    ...
    fleet_copy = Fleet(False, generators_copy, storages_copy)
...
def assign_precharging_values(
    ...
    for storage in fleet_instance.storages.values():
        ...
        storage.stored_energy_temp_forward = (
            storage.stored_energy[interval-1] - max(storage.dispatch_power[interval], 0) / storage.discharge_efficiency
            * resolution - min(storage.dispatch_power[interval], 0) * storage.charge_efficiency * resolution
        )
