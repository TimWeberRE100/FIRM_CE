...
@njit(fastmath=FASTMATH)
def get_annual_limit(generator_instance: Generator_InstanceType, year: int64,) -> None:
...
def dispatch(generator_instance: Generator_InstanceType, interval: int64, merit_order_idx: int64) -> None:
    if merit_order_idx == 0:
        ...
        generator_instance.dispatch_power[interval] = min(
            max(
                generator_instance.node.netload_t - generator_instance.node.storage_power[interval]
...
def update_precharging_flags(
    ...
    generator_instance.trickling_flag = (
        (generator_instance.remaining_trickling_reserves > 1e-6) and generator_instance.trickling_flag
    )
