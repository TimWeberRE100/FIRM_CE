    ...
    if merit_order_idx == 0:
        storage_instance.dispatch_power[interval] = (
            max(
                ...
                max(
                    storage_instance.node.netload_t, -storage_instance.charge_max_t
                ), 0.0
            )
        )
    else:
...
def calculate_fixed_costs(
    ...
    ltcosts_m.calculate_annualised_build(
        ...
        storage_instance.new_build_p, 0.0,
...
def assign_precharging_reserves(storage_instance: Storage_InstanceType) -> None:
    ...
    if storage_instance.precharge_flag:
        storage_instance.precharge_energy = max(
            storage_instance.stored_energy_temp_reverse - storage_instance.stored_energy_temp_forward,
            0.0
        )
    else:
...
def update_precharging_flags(
    ...
    storage_instance.trickling_flag = (
        (storage_instance.remaining_trickling_reserves > 1e-6) and storage_instance.trickling_flag
    )
    storage_instance.precharge_flag = (
        (storage_instance.stored_energy[interval] + 1e-6 < storage_instance.energy_capacity)
        and (storage_instance.precharge_energy > 1e-6) and storage_instance.precharge_flag
    )
    return None
    ...
    if dispatch_power_original > 0.0:  # If originally discharging
        if dispatch_power_update > 0.0:  # Increase discharging power
            ...
            dispatch_energy_update = (
                min(
                    dispatch_power_original,
                    -dispatch_power_update
                ) / storage_instance.discharge_efficiency * resolution
                - min(
                    dispatch_power_original + dispatch_power_update,
                    0.0
                ) * storage_instance.charge_efficiency * resolution
            )
    ...
    # If originally charging
        ...
        if dispatch_power_update > 0.0:  # Reduce charging power and increase discharging power
            dispatch_energy_update = (
                ...
                + min(dispatch_power_original + dispatch_power_update, 0.0) / storage_instance.discharge_efficiency
                * resolution
            )
