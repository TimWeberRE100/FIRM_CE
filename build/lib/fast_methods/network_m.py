...
def set_node_fills_and_surpluses(
    ...
    if transmission_case == 'surplus':
        ...
        for node in network_instance.nodes.values():
            ...
            node.surplus = -min(node.netload_t - max(
                node.storage_power[interval], 0.0) - node.flexible_power[interval], 0
            )
...
def assign_storage_merit_orders(
    ...
    """storages_typed:  # DictType(int64, Storage_InstanceType) TODO: Fix
    """
    for node in network_instance.nodes.values():
...
def assign_flexible_merit_orders(
    ...
    """generators_typed_dict: # DictType(int64, Generators_InstanceType) TODO: Fix
    """
    for node in network_instance.nodes.values():
...
def check_precharging_end(network_instance: Network_InstanceType, interval: int64) -> boolean:
    ...
    for node in network_instance.nodes.values():
        if (
            node.residual_load[interval-1] - node.imports[interval-1] - node.exports[interval-1]
            - node.storage_power[interval-1] - node.flexible_power[interval-1] > 1e-6
        ):
            return False
    return True
...
def update_imports_exports_temp(network_instance: Network_InstanceType, interval: int64) -> None:
    for node in network_instance.nodes.values():
        # Is the below a minus or plus?
        node.imports_exports_update = node.imports_exports_temp - node.imports[interval] - node.exports[interval]
