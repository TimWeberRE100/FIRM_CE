...
class Statistics:
    ...
    def generate_capacities_file(self) -> ResultFile:
        header = []
        data_array = np.empty(
            self.get_asset_column_count(include_minor_lines=True, include_energy_limits=False), dtype=np.float64
        )
        ...
        result_file = ResultFile("capacities", self.results_directory, header, [data_array], decimals=3)
        return result_file
