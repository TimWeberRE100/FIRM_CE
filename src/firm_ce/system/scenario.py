...
from firm_ce.io.file_manager import DataFile, expand_year_dict
...
class Scenario:
    ...
    datafile_filenames_dict (Dict[str, Dict[str, str]]): Raw data imported from the `datafiles.csv` config file. Each row of
        the config file is associated with an item in datafile_filenames_dict.
    ...
    def __init__(self, model_data: ModelData, scenario_id: int) -> None:
        ...
        None.
        """
        self.results_dir = model_data.results_dir
        ...
        firstyear = int(self.scenario_data.get("firstyear"))
        finalyear = int(self.scenario_data.get("finalyear"))

        self.network = construct_Network_object(
            self.get_scenario_dicts(model_data.nodes, firstyear, finalyear),
            self.get_scenario_dicts(model_data.lines, firstyear, finalyear),
        ...
        self.fleet = construct_Fleet_object(
            ...
            self.get_scenario_dicts(model_data.fuels, firstyear, finalyear),
    ...
    def get_scenario_dicts(
        self,
        imported_dict: Dict[int, Dict[str, Any]] | Dict[int, Dict[int, Dict[str, Any]]],
        firstyear: int = None,
        finalyear: int = None,
        ...
        For multiyear files (where values are year-keyed dicts), any asset row stored under the
        YEAR_ALL_STR year key is expanded to cover every year in [firstyear, finalyear] that does not
        already have a specific entry.

        Parameters:
        -------
        imported_dict: Raw data imported from a config file for the Model. Either a flat
            Dict[int, Dict[str, Any]] (for non-multiyear files) or a nested
            Dict[int, Dict[int | str, Dict[str, Any]]] (for multiyear files such as generators,
            storages, fuels, lines, and nodes).
        firstyear (int): First year of the modelling horizon, used to expand "all" year entries.
            Defaults to None (no expansion performed).
        finalyear (int): Final year of the modelling horizon, used to expand "all" year entries.
            Defaults to None (no expansion performed).

        Returns:
        -------
        Filtered data from the config file assigned to this Scenario instance, keyed by model-level ID.
        """
        result = {}
        for asset_id, value in imported_dict.items():
            # Multi-year format
            if isinstance(next(iter(value.values())), dict):
                any_year_data = next(iter(value.values()))
                scenarios = parse_comma_separated(any_year_data.get("scenarios", ""))
                if self.name in scenarios or scenarios == [SCENARIOS_ALL_STR]:
                    result[asset_id] = expand_year_dict(value, firstyear, finalyear)
            else:
                # Single-year format
                scenarios = parse_comma_separated(value.get("scenarios", ""))
                if self.name in scenarios or scenarios == [SCENARIOS_ALL_STR]:
