from firm_ce.system.components import Fleet, Generator_InstanceType, Storage_InstanceType, Fleet_InstanceType
...
from firm_ce.common.typing import TypedDict, int64, DictType, float64, unicode_type, boolean
from firm_ce.common.jit_overload import njit

@njit(fastmath=FASTMATH)
def create_dynamic_copy(fleet_instance: Fleet_InstanceType, 
...
@njit(fastmath=FASTMATH)
def build_capacities(fleet_instance: Fleet_InstanceType, 
                     decision_x: float64[:], 
...
@njit(fastmath=FASTMATH)    
def update_stored_energies(fleet_instance: Fleet_InstanceType, interval: int64, resolution: float64, forward_time_flag: boolean) -> None:
    for storage in fleet_instance.storages.values():
        storage_m.update_stored_energy(storage, interval, resolution, forward_time_flag)
    return None

@njit(fastmath=FASTMATH)    
def update_remaining_flexible_energies(fleet_instance: Fleet_InstanceType, 
                                       interval: int64, 
                                       resolution: float64, 
...
@njit(fastmath=FASTMATH)
def assign_precharging_values(fleet_instance: Fleet_InstanceType, 
                              interval: int64, 
                              resolution: float64,
                              year: int64) -> None:
    for generator in fleet_instance.generators.values():
        if generator_m.check_unit_type(generator, 'flexible'):
            generator.remaining_energy_temp_forward = generator.remaining_energy[interval-1] - generator.dispatch_power[interval] * resolution
            generator.remaining_energy_temp_forward = min(
                max(generator.remaining_energy_temp_forward, 0.0), 
                generator_m.get_annual_limit(generator, year)
            )
    ...
    for storage in fleet_instance.storages.values():
        ...
        storage.stored_energy_temp_forward = storage.stored_energy[interval-1] \
...
def determine_feasible_storage_dispatch(fleet_instance: Fleet_InstanceType, interval: int64) -> boolean:
    ...
    for storage in fleet_instance.storages.values():
        ...
        storage.dispatch_power[interval] = (
            max(min(original_dispatch_power, storage.discharge_max_t), 0.0) +
            min(max(original_dispatch_power, -storage.charge_max_t), 0.0)
        )
...
def determine_feasible_flexible_dispatch(fleet_instance: Fleet_InstanceType, interval: int64) -> boolean:
    ...
    for generator in fleet_instance.generators.values():
        ...
        generator.dispatch_power[interval] = (
            min(original_dispatch_power, generator.flexible_max_t)
        )
