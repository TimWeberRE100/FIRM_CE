...
from firm_ce.common.typing import int64, DictType, float64, unicode_type, boolean

@njit(fastmath=FASTMATH)
def create_dynamic_copy(storage_instance: Storage_InstanceType, 
...
@njit(fastmath=FASTMATH)
def build_capacity(storage_instance: Storage_InstanceType, 
                   new_build_capacity: float64, 
                   capacity_type: unicode_type) -> None:
...
@njit(fastmath=FASTMATH)    
def allocate_memory(storage_instance: Storage_InstanceType, 
                    intervals_count: int64) -> None:
    if storage_instance.static_instance:
        raise_static_modification_error()
...
@njit(fastmath=FASTMATH)    
def set_dispatch_max_t(storage_instance: Storage_InstanceType, 
                       interval: int64, 
                       resolution: float64, 
    ...
    if forward_time_flag:
        ...
        storage_instance.discharge_max_t = min(
            storage_instance.power_capacity, 
            (storage_instance.energy_capacity - storage_instance.stored_energy_temp_reverse) * storage_instance.discharge_efficiency / resolution
        )
        storage_instance.charge_max_t = min(
            storage_instance.power_capacity, 
...
@njit(fastmath=FASTMATH)    
def dispatch(storage_instance: Storage_InstanceType, 
             interval: int64, 
...
@njit(fastmath=FASTMATH)    
def update_stored_energy(storage_instance: Storage_InstanceType, 
                         interval: int64, 
                         resolution: float64, 
                         forward_time_flag: boolean) -> None:
    if forward_time_flag:
...
@njit(fastmath=FASTMATH)    
def calculate_lt_discharge(storage_instance: Storage_InstanceType, 
...
@njit(fastmath=FASTMATH)
def initialise_deficit_block(storage_instance: Storage_InstanceType, 
                             interval: int64) -> None:
...
@njit(fastmath=FASTMATH)
def update_deficit_block_bounds(storage_instance: Storage_InstanceType, 
                                stored_energy: float64) -> None:
...
def assign_precharging_reserves(storage_instance: Storage_InstanceType) -> None:
    storage_instance.precharge_flag = (
        storage_instance.deficit_block_max_storage - storage_instance.deficit_block_min_storage > storage_instance.stored_energy_temp_forward
    )
    if storage_instance.precharge_flag:
        storage_instance.precharge_energy = max(storage_instance.stored_energy_temp_reverse - storage_instance.stored_energy_temp_forward, 0.0)
    else:
        storage_instance.precharge_energy = 0.0
    storage_instance.trickling_reserves = storage_instance.deficit_block_max_storage - storage_instance.deficit_block_min_storage
    return None
...
@njit(fastmath=FASTMATH)
def update_precharging_flags(storage_instance: Storage_InstanceType, 
                             interval: int64) -> None:
...
@njit(fastmath=FASTMATH)
def set_precharging_max_t(storage_instance: Storage_InstanceType, 
                          interval: int64, 
                          resolution: float64, 
                          merit_order_idx: int64) -> None:
    ...
    if storage_instance.trickling_flag:
        ...
        charge_reduction_constraint_energy = charge_reduction_constraint_power * storage_instance.charge_efficiency * resolution
        discharge_increase_constraint_power = min(
            (storage_instance.trickling_reserves - charge_reduction_constraint_energy) * storage_instance.discharge_efficiency / resolution,
    ...
    if storage_instance.precharge_flag:
        ...
        discharge_reduction_constraint_energy = discharge_reduction_constraint_power / storage_instance.discharge_efficiency * resolution
        charge_increase_constraint_power = min(
            (storage_instance.precharge_energy - discharge_reduction_constraint_energy) / storage_instance.charge_efficiency / resolution,
...
@njit(fastmath=FASTMATH)
def calculate_dispatch_energy_update(storage_instance: Storage_InstanceType, 
    ...
    if dispatch_power_original > 0.0: # If originally discharging
        if dispatch_power_update > 0.0: # Increase discharging power
            ...
            dispatch_energy_update = (
                min(dispatch_power_original, -dispatch_power_update) / storage_instance.discharge_efficiency * resolution 
                - min(dispatch_power_original + dispatch_power_update, 0.0) * storage_instance.charge_efficiency * resolution
            )
    ...
    # If originally charging
        ...
        if dispatch_power_update > 0.0: # Reduce charging power and increase discharging power
             dispatch_energy_update = (
                ...
                + min(dispatch_power_original + dispatch_power_update, 0.0) / storage_instance.discharge_efficiency * resolution
            )
...
@njit(fastmath=FASTMATH)
def update_precharge_dispatch(storage_instance: Storage_InstanceType, 
                              interval: int64, 
                              resolution: float64, 
                              dispatch_power_update: float64, 
                              precharging_flag: boolean, 
                              merit_order_idx: int64) -> None:  
    dispatch_energy_update = calculate_dispatch_energy_update(storage_instance, storage_instance.dispatch_power[interval], dispatch_power_update, resolution)
